...
export async function render(outlet, { navigate }) {
      ...
      <div class="login-card">
        ...
        <form class="login-form" data-part="form" novalidate>
          ...
          <div class="field">
            ...
            <p class="field-help">رمز عضویت با شناسه عمومی مانند ST-… متفاوت است.</p>
  ...
  const showAlert = (message) => {
    alert.innerHTML = `${icon("alert", { size: 18 })}<span>${escapeHtml(message)}</span><button type="button" class="login-alert-close" data-action="dismiss-alert" aria-label="بستن">${icon("close", { size: 15 })}</button>`;
    alert.hidden = false;
  };
  const clearAlert = () => {
    alert.hidden = true;
  ...
  alert.addEventListener("click", (event) => {
    if (event.target.closest('[data-action="dismiss-alert"]')) clearAlert();
  });
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      navigate(data?.user?.role === "student" ? ROUTES.home : ROUTES.admin, {
        replace: true,
      });
    } catch (err) {
      const message = err.userMessage ?? "ورود انجام نشد. دوباره تلاش کن.";
      showAlert(message);
      toastError(message);
      input.setAttribute("aria-invalid", "true");
      input.select();
