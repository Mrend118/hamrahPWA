...
import { formatDuration, minutesToMs, toFa } from '../utils/format-time.js';
  ...
  return `<div class="podium">
    ${top3.map((item, index) => {
      ...
      return `<div class="podium-slot" data-place="${place}">
        ...
        <span class="time num">${formatDuration(minutesToMs(item.minutes))}</span>
      </div>`;
    }).join('')}
  </div>`;
}

/** یک سطر از جدول رتبه‌بندی */
export function renderRankRow(item, { me = false } = {}) {
  return `<div class="rank-row" data-me="${me}">
    ...
    <span class="rank-meta">
      <span>${formatDuration(minutesToMs(item.minutes))}</span>
