const PATHS = {
  ...
  stats: '<path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/>',
  trending:
  ...
  spark:
    '<path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5L11.52 2.365a.5.5 0 0 1 .96 0l1.582 6.135A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"/>',
};

export function icon(name, options = {}) {
  const { size = 20, className = "", strokeWidth = 1.75, title } = options;
  const body = PATHS[name];
  if (!body) {
    console.warn(`[icons] آیکن «${name}» تعریف نشده است.`);
    return "";
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24"
    fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round"
    class="icon ${className}" ${title ? `role="img" aria-label="${title}"` : 'aria-hidden="true" focusable="false"'}>${body}</svg>`;
}
